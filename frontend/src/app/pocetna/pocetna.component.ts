...
export class PocetnaComponent implements OnInit{
  ...
  dohvatiSveKorisnike(){
    this.servis.getAllUsers().subscribe(data=>{
      ...
      this.pronadjiNajslicnijeKorisnike(); // Dodao
    })
  } 
  ...
  najslicnijiKorisnici:User[] = []; // Dodao

  napraviVektor(kor : User) : number[]{

    let podaci = [kor.godine, kor.lokacija.lat, kor.lokacija.lon];
    let min = Math.min(...<number[]>podaci);
    let max = Math.max(...<number[]>podaci);
    podaci = podaci.map((elem)=>{return (<number>elem-min) / (max-min)}); // Normalizujemo 
  ...
  kreirajKorisnickiVektor(destinacije):number[]{
      if(destinacije.length == 0){ // Dodao da pokrijemo slucaj kada korisnik nije posetio ni jednu atrakciju
